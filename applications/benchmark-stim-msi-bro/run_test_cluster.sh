...
if [ "${PLOT:-1}" != "0" ]; then
  ./.venv/bin/python applications/benchmark-stim-msi-bro/plot_bro_heatmap.py \
    --csv applications/benchmark-stim-msi-bro/benchmark_bro_results_p1.0e-03_r10000.csv \
    || echo "!! heatmap generation failed -- the CSV is still in applications/benchmark-stim-msi-bro"
fi
