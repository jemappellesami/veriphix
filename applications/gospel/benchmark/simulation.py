...
-----------------------------------
    python applications/gospel/benchmark/simulation.py
...
SAMPLED_BASE_SMALL = Path("applications/gospel/sampled_circuits-small")


@app.command()
def main(
    sampled_base: Annotated[Path | None, typer.Option(help="Circuit folder (auto: small locally, full on cluster)")] = None,
    test_rounds:  Annotated[int,  typer.Option()] = 100,
    out_csv:      Annotated[Path, typer.Option()] = Path("applications/gospel/benchmark/results/results-FK.csv"),
    ...
    is_cluster = not (walltime is None and memory is None and cores is None)
    if sampled_base is None:
        sampled_base = SAMPLED_BASE if is_cluster else SAMPLED_BASE_SMALL
    typer.echo(f"Using circuits from: {sampled_base}")
    folders = sorted(
